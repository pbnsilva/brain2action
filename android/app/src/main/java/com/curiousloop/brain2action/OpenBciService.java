...
public class OpenBciService extends Service {
    ...
    private static final String ACTION_USB_PERMISSION = "com.curiousloop.brain2action.USB_PERMISSION";
            ...
            try {
                if(!isStreaming) {
                    ...
                    for(int i=0; i<arg0.length; i++) {
                        ...
                        if(currentSampleByteCount == 33) {
                            if(readSample(currentSample)) {
                                sampleCount += 1;
                            }
                        }
                    }
                    if (handler != null)
                        handler.obtainMessage(MESSAGE_FROM_SERIAL_PORT, sampleCount + "\n").sendToTarget();
                }
    ...
    private boolean readSample(byte[] data) {
