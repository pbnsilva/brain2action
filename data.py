...
class CSVMotionDataSource(MotionDataSource):

    def __init__(self, csv_file, preload=False):
...
class LiveMotionDataSource(MotionDataSource):

    def __init__(self, port):
        self._port = port


class BrainDataSource:
...
class CSVBrainDataSource(BrainDataSource):

    def __init__(self, csv_file):
        self._csv_file = csv_file


class LiveBrainDataSource(BrainDataSource):
...
class RawOpenBCI(_BaseRaw):
    ...
    def _read_segment_file(self, data, idx, fi, start, stop, cals, mult):
        ...
        # data[:, offset:offset + stop - start] = np.dot(mult, data_[idx]) if mult is not None else data_[idx]
#         print start, stop, data_.shape, data.shape
        data[:,:] = np.dot(mult, data_[idx]) if mult is not None else data_[idx]
        data[:,:] *= 1e-6
#         m = np.mean(data[:,:],axis=1)
#         data[:,:] -= np.array([m] * len(data[0,:])).T
